...
namespace GoFish
{
    ...
    public class Player : IEquatable<Player>
    {
        ...
        {
            if (Frags.Count != values.Count)
            {
                Debug.LogError($"Displaying cards count {Frags.Count} is not equal to card values count {values.Count} for {PlayerId}");
                return;
            }

            for (int index = 0; index < values.Count; index++)
            {
                Card card = Frags[index];
